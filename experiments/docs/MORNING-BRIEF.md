...
### M-38 · 演化器不知道 `file:` 路径该怎么写(**这是停跑重发的理由**)
...
最重的一条:同一份 brief 要求**加工具的候选必须提供 Level-2 往返证据**;
工具没注册就永远拿不到那个观察 ⇒ **论文四个演化杠杆里的"工具"杠杆近乎结构性失效**,
而论文自己 GAIA 上最大的单项改进恰是一次工具编辑。

**验证两层**:单元 13 项(核心是**验 brief 说的是真话** —— 教错格式比不教更糟);
**实跑探针**同模型各 10 次:**无 brief 两斜杠 0/10 → 有 brief 10/10**。
