...
- **✅ 裁定 C/D/E 落定(Jul-31,用户令"E拉满,cd 都不需要")**:
  ...
  - **裁 D = 臂推迟,但触发规则现在写死(预注册)**:跑完量 token,
    **B2 总 token > A1 × 1.2 ⇒ 必须补配平臂**;≤1.2 ⇒ 直接报双方 token 数
    (等预算下取胜=最强结果形态)。⚠️注:effort 拉满会放大 B2 的多次调用×高推理
    token 效应,该规则触发概率上升,须在 B-FREEZE 注明。
  ...
  - 最小首发阵容:**A1(high 补测)+ B0 + B2**;条件臂:B1(归因)/ 配平臂(token 规则)。
