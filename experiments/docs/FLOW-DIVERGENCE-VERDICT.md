...
5. 其余 **19 条全部 DECLARED**(23 − 3 NEW − 1 MISDECLARED = 19,非约数),四分类以 **(b) 留白填补**为主(U1–U13 逐条落地),少量 (a)/(d) 已被 M-04/M-17/M-19/§7.8 精确覆盖。
6. **总体判语:申报完备度高。** 口径须写明,否则同一事实可报出两个数(Jul-31 勘误):**申报且口径正确 = 19/23 = 82.6%**(即下文「约 83%」的来源);**台账中存在条目者 = 20/23 = 87.0%**(把那 1 条 MISDECLARED 也算进去);**完全未申报 = 3/23 = 13.0%**(NEW-1..3)。引用时必须带上是哪一种口径,单说「约 83%」或「约 87%」都不完整。多数已申报条目带预注册 ablation。 台账的短板不是漏报大偏差,而是 (i) 三行时效滞后、(ii) 一处 verbatim 过度声明掩盖了 Ensemble 回退基线的跨变体耦合。**默认配置复现的是 Global 冒烟档而非论文旗舰 Ensemble——此点被 GAP-AUDIT §0 显著且正确地申报**,是台账质量最强的一处。
