...
def test_normalize_file_uri_windows_forms(value, expected):
    # The builder's per-target resolver converged onto the shared
    # harnessx.core.file_uri.normalize_file_uri (one resolver for every loader).
    from harnessx.core.file_uri import normalize_file_uri

    assert normalize_file_uri(value) == expected


@pytest.mark.parametrize("value,expected", _POSIX_PATH_FORMS)
def test_normalize_file_uri_posix_forms(value, expected):
    from harnessx.core.file_uri import normalize_file_uri
