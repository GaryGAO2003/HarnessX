...
**身份判定还堵了第二个洞**：config.yaml 会用 `file://…/R<N>/applied/<cid>/x.py`
和 `template_path` 引用演化出的资产，配置文件相同而资产被改动也会让"同配置"落空。
逐一核对 L0 R6/R7/R8 引用的 7 份资产（2 工具、4 处理器、1 提示词模板），
mtime 全部 ≤ 08-12 23:05，**早于 R6 任务阶段开始的 23:20**，期间无一改动。
因此该三连是配置文件同一 + 引用资产同一的双重意义上的同配置。

（两臂各 10 轮跑满后的最终表，n=9 个同配置轮对）

| 臂 | 轮对（同配置） | 变好 | 变坏 | 净 | churn | churn 率 |
|---|---|---|---|---|---|---|
| L0 | R3→R4 | +14 | −11 | +3 | 25 | 24% |
| **L2** | **R8→R9** | **+10** | **−15** | **−5** | **25** | **24%** |
...
**churn 均值 22.0 题，中位 23，范围 18–25（17–24%）。总分只动 −5…+6。**

L2 的 **R8→R9** 是收官时新增的一个独立样本：该轮因算子续跑而未执行 meta，
配置与 R8 逐字节相同（组 `f3ddd112e6`），结果 77→72，**净 −5 题、churn 25 题**。
它是在本文其余结论全部写完之后才产生的数据，事前不可能被挑选——
**对同配置churn 这条主张，它是一次干净的样本外验证**。
...
两臂全轮均未解出的只有 **6 题**（仅 L0 未解 3 题、仅 L2 未解 5 题）。

**读法**：可靠解出的只有三分之一，可靠解不出的不到一成，**其余近六成每轮都在掷硬币**。
每轮约 20 题的同配置churn 就是活动带的三分之一在换位。这一条解释了本文其余全部现象——
噪声包络、峰值落在重复组高端、回归账列出的"回归"——它们都是同一个事实的不同侧面。

**对读数的直接后果**：任何按"某题被修好/被弄坏"来归因的机制（回归账、
ship 的 predicted-unlock 清单、Critic 的"覆盖了 N 条回归"论证）在活动带内**先天不可靠**，
除非其效应量能压过 0.6 左右的单题通过率波动。要在这种床上做逐题归因，
必须先对同一配置多次抽样再比，或把判据换成跨多轮的持续状态。

## 1b. 由重复组直接算出的噪声模型与本设计的分辨率

每个同配置组都是同一配置的重复测量，组内对组均值的偏差即单轮噪声
（组间差是配置效应，不参与）。合并六组：

| 量 | 值（10 轮跑满，7 个重复组） |
|---|---|
| 单轮 SD | **2.52 题 = 2.44pp**（df=8） |
| 两轮之差的 SD | 3.56 题 |
| 本设计能判定的最小臂间差（双侧 α=.05，~80% power） | **10.0 题 = 9.7pp** |

**后果**：103 题 × 单种子 × 单轮比较的协议，只能分辨 ~9pp 以上的臂间差。
两臂终值 73 与 72 只差 1 题（峰值差 3 题），远在分辨率以下——**本战役无法分离两臂**，
这是设计的分辨率问题，不是"证明了 L2 无效"。
（L2 的证据载荷另有结构性问题，见 `GHX-L2-EMPTY-CONE-ROOT-CAUSE.md`；
两条互相独立：即便载荷修好，这个协议仍看不见小于 9pp 的效应。）

**与既往测量对账（不可只报有利的那个）**：`ghx_103x3_t1`（2026-08-11，pool era）
对同一 genotype `dbecb411` 测过三次，raw 54 / 59 / 65 题，**带宽 11 题**，
由此定的判读规则是"APPLY 幅度 < ±5 题视为未证实"。本战役测得的带宽（max 6 题）
与 pooled SD（2.34 题）都更窄。差异的可能来源：那三次是**判分加固之前**跑的
（当时 deepseek-chat judge 对无答案轨迹约 40% 假阳性，见 L0 基线记录），
判分噪声叠在了配置噪声上。

**保守取值**：论文用两者中更宽的一个作判读阈——即沿用 ±5 题 / 带宽 11 的旧包络
作上界，本战役的 2.34 题 SD 作下界，并把两次测量的条件差异写进 threats to validity。
不得只引用本战役这个更窄的数字。

想要看见 3–5pp 量级的效应，须在下列方向上加：多种子、pass@k、
跨轮配对（同一配置多次抽样后取均值再比），或直接扩题库。
df=7 意味着 SD 本身也不精确，上表按估计值读，不当精确常数用。
